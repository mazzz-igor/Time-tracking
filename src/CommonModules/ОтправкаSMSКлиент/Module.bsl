...
Процедура СоздатьНовоеSMSПроверкаНастроекВыполнена(ОтправкаSMSНастроена, ПараметрыОтправки) Экспорт
	...
	Если ОтправкаSMSНастроена Тогда
		...
		Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.Взаимодействия")
			...
			ИсточникКонтактнойИнформации = ?(ПараметрыОтправки.ДополнительныеПараметры.Свойство("ИсточникКонтактнойИнформации"), ПараметрыОтправки.ДополнительныеПараметры.ИсточникКонтактнойИнформации, "");
			...
			Для каждого ЭлементМассива Из ПараметрыОтправки.НомераПолучателей Цикл
				...
				Если ТипЗнч(ЭлементМассива) = Тип("Строка") Тогда
					НомерТелефона                = ЭлементМассива;
					Представление                = ЭлементМассива;
					ИсточникКонтактнойИнформации = ИсточникКонтактнойИнформации;
				Иначе
					НомерТелефона                = ЭлементМассива.НомерТелефона;
					Представление                = ЭлементМассива.Представление;
					ИсточникКонтактнойИнформации = ЭлементМассива.ИсточникКонтактнойИнформации;
				КонецЕсли;
				
				Адресаты.Добавить(Новый Структура("Телефон, Представление, ИсточникКонтактнойИнформации",
				НомерТелефона, Представление, ИсточникКонтактнойИнформации));
